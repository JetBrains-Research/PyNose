package org.jetbrains.research.pynose.cli;
...
public class PluginRunner implements ApplicationStarter {
    ...
    public String getCommandName() {
        return "org/jetbrains/research/pynose";
    }
    ...
    public void main(@NotNull List<String> args) {
        ...
        if (p.isInitialized()) {
            ...
            var projectPsiFiles = PyNoseUtils.extractPsiFromProject(p);
            projectPsiFiles.forEach(psiFile -> {
                var testCaseResultArray = new JsonArray();
                PyNoseUtils.gatherTestCases(psiFile).forEach(testCase -> {
                    ...
                    PyNoseUtils.newAllDetectors(testCase).forEach(detector -> {
                        detector.analyze();
                        detectorResultArray.add(detector.getSmellDetailJSON());
                    });
                    testCaseResultObject.add("detectorResults", detectorResultArray);
                    var testMethodCount = PyNoseUtils.gatherTestMethods(testCase).size();
